# Change - Implement Phase 5.2 Parquet dataset preview
...
Phase 5.1에서 제외했던 Parquet dataset preview를 같은 HF-like UI preview surface에 추가하기 위해 진행했다.

## How

pyarrow 기반 Parquet parser, MinIO range reader, Parquet byte cap, schema source_type, static UI source type rendering, Parquet seed/check/runbook/report updates를 추가했다. Upload와 HF Hub API compatibility는 포함하지 않았다.

## Files

- deploy/hf-ui/app/main.py
- deploy/hf-ui/app/requirements.txt
...
- docker compose -f deploy/compose/hf-ui/docker-compose.yml config with example env files loaded
- docker build -t lab/hf-ui:phase5.2-preview deploy/hf-ui/app
- docker run in-image preview assertions for JSONL/CSV/JSON/Parquet and Parquet 413 cap
- git diff --check
