...
GRIST_CONTAINER_UID="${GRIST_CONTAINER_UID:-1000}"
GRIST_CONTAINER_GID="${GRIST_CONTAINER_GID:-1000}"
...
run install -d -m 0750 "$LAB_PLATFORM_ROOT"/data/{postgres,redis,minio,authentik,gitea,nextcloud,grist,overleaf}
run install -d -m 0770 "$LAB_PLATFORM_ROOT"/data/authentik/{media,certs,custom-templates}
run install -d -m 0770 "$LAB_PLATFORM_ROOT"/data/grist/persist
...
if [[ "$DRY_RUN" == "true" ]]; then
  ...
  echo "+ chown -R ${GRIST_CONTAINER_UID}:${GRIST_CONTAINER_GID} $LAB_PLATFORM_ROOT/data/grist"
else
  ...
  chown -R "${GRIST_CONTAINER_UID}:${GRIST_CONTAINER_GID}" "$LAB_PLATFORM_ROOT"/data/grist
fi
