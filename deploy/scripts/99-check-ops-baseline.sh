...
INCOMING_LABSTACK_INCLUDE_MLFLOW="${LABSTACK_INCLUDE_MLFLOW:-}"
load_envs \
...
LABSTACK_INCLUDE_MLFLOW="${LABSTACK_INCLUDE_MLFLOW:-false}"
...
[[ -n "$INCOMING_LABSTACK_INCLUDE_MLFLOW" ]] && LABSTACK_INCLUDE_MLFLOW="$INCOMING_LABSTACK_INCLUDE_MLFLOW"
...
check_active_containers() {
  ...
  if is_true "$LABSTACK_INCLUDE_MLFLOW"; then
    containers+=(mlflow)
  fi
...
check_strict_blockers_are_visible() {
  ...
  for name in ROOT_DOMAIN AUTH_DOMAIN HULY_DOMAIN FILES_DOMAIN S3_DOMAIN HF_DOMAIN MLFLOW_DOMAIN OVERLEAF_DOMAIN; do
